...
import { roundGPA } from '../../lib/gpaCalculator';
...
export function SemesterSection({
  ...
  const gpaDisplay = roundGPA(semesterGPA);

  return (
    <div className="border border-[var(--border)] rounded-[8px] overflow-hidden mb-3 last:mb-0">
      {/* Header */}
      <div
        className="flex items-center justify-between px-4 py-3 bg-[var(--bg-surface)] cursor-pointer hover:bg-[var(--bg-hover)] transition-colors select-none"
        ...
        <div className="flex items-center gap-3">
          {gpaDisplay !== null && (
            <span className="text-[12px] text-[var(--text-secondary)]">
              GPA: <span className="font-medium tabular-nums text-[var(--text-primary)]">{gpaDisplay.toFixed(2)}</span>
            </span>
          )}
          <span className="text-[12px] text-[var(--text-tertiary)] tabular-nums">
            {semester.courses.reduce((s, c) => s + c.creditHours, 0)} cr
          </span>
          <button
            type="button"
            onClick={(e) => {
            ...
            className="opacity-0 group-hover:opacity-100 w-5 h-5 flex items-center justify-center text-[var(--text-tertiary)] hover:text-[var(--danger)] transition-colors"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--bg-raised)]">
                ...
                {scenarioMode && (
                  <th className="text-left py-2 px-2 text-[11px] font-medium text-[var(--text-tertiary)]">Scenario</th>
                )}
            ...
            <tbody>
              {semester.courses.map((course) => (
                <tr key={course.id} className="border-b border-[var(--border-subtle)] last:border-0 group">
                  <td colSpan={scenarioMode ? 9 : 8} className="p-0">
                    <table className="w-full">
                      <tbody>
                        <CourseRow
                          course={course}
